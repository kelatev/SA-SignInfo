...
function getURLParameter(url, name, defaultValue) {
    ...
    if (url.indexOf("?") == -1) return defaultValue;

    var param = defaultValue;
    var paramsString = url.substring(url.indexOf("?") + 1, url.length);
    var params = paramsString.split("&");
    for (var i = 0; i < params.length; i++) {
        if (params[i].indexOf(name + "=") != 0) continue;

        param = params[i].substring((name + "=").length, params[i].length);
        break;
    }
...
var EU_LOG_EVENTS = getURLParameter(location.href, "log", "false") == "true" ? true : false;
var EU_MAX_DATA_SIZE_MB = parseInt(getURLParameter(location.href, "maxFileSize", "5"));
...
var EUVerifyResult = function (resultCode, signFile, dataFile, signType, signsInfos, data) {
...
function MakeURL(url) {
    ...
    if (s_origin && s_pathname) {
        return s_origin + s_pathname.substr(0, s_pathname.lastIndexOf("/")) + "/" + url;
    }
...
function DowloadData(url, dataType) {
    try {
        ...
        if (dataType == "binary") xmlHttp.responseType = "arraybuffer";
        xmlHttp.send();
        if (xmlHttp.status != 200) {
            throw "Download data error. URL - " + url + ", status - " + xmlHttp.status;
        }
...
function SetXMLHTTPDirectAccess(cas) {
    try {
        var _addDNSName = function (uri, dnsNames) {
            ...
            uri = uri.indexOf("://") > -1 ? uri.split("/")[2] : uri.split("/")[0];

            if (dnsNames.indexOf(uri) >= 0) return;
        ...
        for (var i = 0; i < cas.length; i++) {
            if (!cas[i].directAccess) continue;
            ...
            _addDNSName(cas[i].ocspAccessPointAddress, directAccessURLs);
        }
...
function SetSettings(cas, httpProxyServiceURL, directAccess, tslAddress) {
    ...
    if (directAccess) SetXMLHTTPDirectAccess(cas);

    euSign.SetRuntimeParameter(EU_CHECK_CERT_CHAIN_ON_SIGN_TIME_PARAMETER, true);
...
function IsSignerDigitalTimeStamp(info) {
    return (
        info.GetExtKeyUsages().indexOf(UA_OID_EXT_KEY_USAGE_STAMP) > -1 ||
...
function IsUseCRLs(issuerCN) {
    if (!s_settings.CRLs) return false;
...
function IsQSCDSNInCert(issuerCN) {
    for (var i = 0; i < s_settings.CAs.length; i++) {
        ...
        if (CA.qscdSNInCert && CA.issuerCNs.indexOf(issuerCN) > -1) {
            return true;
        }
    }
...
function GetQSCD(cert) {
    ...
    try {
        ...
        if (qscd.use) {
            ...
            (infoEx.GetPublicKeyType() == EU_CERT_KEY_TYPE_ECDSA ||
                infoEx.GetPublicKeyType() == EU_CERT_KEY_TYPE_RSA) &&
...
function Initialize(data) {
    try {
        ...
        s_settings.CACertificates = DowloadData(s_settings.CACertificates, "binary");

        if (EU_LANGUAGE == "") EU_LANGUAGE = s_settings.language;
        euSign.SetErrorMessageLanguage(EU_LANGUAGE);

        euSign.Initialize();
        euSign.SetOCSPResponseExpireTime(s_settings.verifySettings.ocspResponseExpireTime);
        euSign.SetRuntimeParameter(EU_STRING_ENCODING_PARAMETER, EU_UTF8_ENCODING);
        ...
        euSign.SaveCertificates(s_settings.CACertificates);
...
function VerifyExternalData(data, sign) {
    ...
    try {
        ...
        if (signers <= 0) euSign.RaiseError(EU_ERROR_PKI_FORMATS_FAILED);
        ...
        for (var i = 0; i < signers; i++) {
            ...
            var hashContext = euSign.CtxHashDataBegin(context, hashAlgo, signerCert.GetData());
            var parts = Math.floor(data.length / Module.MAX_DATA_SIZE);
            var lastPart = data.length % Module.MAX_DATA_SIZE;
            for (var j = 0; j < parts; j++) {
                euSign.CtxHashDataContinue(
                    hashContext,
                    new Uint8Array(data.buffer, j * Module.MAX_DATA_SIZE, Module.MAX_DATA_SIZE),
                );
            }
            if (lastPart) {
                euSign.CtxHashDataContinue(
                    hashContext,
                    new Uint8Array(data.buffer, parts * Module.MAX_DATA_SIZE, lastPart),
                );
            }

            var hash = euSign.CtxHashDataEnd(hashContext, false);
            var useCRL = IsUseCRLs(signerCert.GetInfoEx().GetIssuerCN());
            var signInfo = euSign.VerifyHashOnTimeEx(hash, i, sign, null, useCRL, !useCRL);
            var signFormat = euSign.GetSignType(i, sign);

            signsInfos.push(
                CreateSignInfoResult(
                    signInfo,
                    signerCert,
                    signFormat,
        ...
        if (context != null) euSign.CtxFree(context);
        throw e;
    }
...
function VerifyDataWithInternalSigns(context, signedData, signsInfos) {
    ...
    try {
        ...
        if (e.errorCode != EU_ERROR_PKI_FORMATS_FAILED) throw e;

        isInternalSign = false;
    }

    if (!isInternalSign) return null;

    signsCount = euSign.CtxGetSignsCount(context, signedData);
    if (signsCount <= 0) euSign.RaiseError(EU_ERROR_PKI_FORMATS_FAILED);
    for (var i = 0; i < signsCount; i++) {
        ...
        signInfo = euSign.VerifyDataInternalOnTimeEx(i, signedData, null, useCRL, !useCRL);
...
function VerifyMultySignedDataInternal(signedData) {
    ...
    try {
        context = euSign.CtxCreate();
        euSign.CtxSetParameter(context, EU_RESOLVE_OIDS_PARAMETER, false);
        while ((_tmpData = VerifyDataWithInternalSigns(context, _tmpData, _signsInfos)) != null) {
...
function VerifyMultiSignFileDataInternal(signedFile, onSuccess, onError) {
    var _onSuccess = function (signedFileData) {
        try {
            ...
            var result = VerifyMultySignedDataInternal(signedData);
            ...
            var signType = EUSignContainerType.Enveloped | EUSignContainerType.CAdES;
            if (signsInfos.length > 1 || (signsInfos.length > 0 && signsInfos[0].length > 1)) {
...
function VerifyBase64FileInternal(signedFile, onSuccess, onError) {
    var _onSuccess = function (signedFileData) {
        var signedData = null;
        try {
            signedData = euSign.Base64Decode(ArrayToString(signedFileData.data));
        } catch (e) {
            onSuccess(
                new EUVerifyResult(
                    EUVerifyErrorCode.InvalidFileFormat,
        ...
        try {
            ...
            if (s_verifyMultiSign) {
                ...
                var signerCert = euSign.CtxGetSignerInfo(context, 0, signedData);
                var useCRL = IsUseCRLs(signerCert.GetInfoEx().GetIssuerCN());
...
function VerifyMultiSignFile(file, fileWithSign, onSuccess, onError) {
    ...
    var _getFileSignParams = function (signIndex, onSuccess, onError) {
        euSign.CtxGetFileSignerInfo(
            context,
            signIndex,
            fileWithSign,
            function (signerCert) {
                euSign.GetFileSignType(
                    signIndex,
                    fileWithSign,
                    function (signType) {
    ...
    var _verify = function (signIndex, count, signsInfos) {
        ...
        var signType = file == null ? EUSignContainerType.Enveloped : EUSignContainerType.Detached;
    ...
    try {
        ...
        euSign.CtxGetFileSignsCount(
            ...
            function (count) {
                if (count <= 0) {
                    onError(euSign.MakeError(EU_ERROR_PKI_FORMATS_FAILED));
                    return;
                }
...
function VerifyFileWithInternalSign(file, onSuccess, onError) {
    ...
    var _onSuccess = function (isAvailable) {
        ...
        VerifyMultiSignFile(null, file, onSuccess, onError);
    };
    ...
    var _onError = function (e) {
        ...
        VerifyBase64FileInternal(file, onSuccess, onError);
    };

    euSign.IsDataInSignedFileAvailable(file, _onSuccess, _onError);
}
...
function VerifyFileWithExternalSign(file, fileWithSign, onSuccess, onError) {
    ...
    var _onSuccess = function (isAvailable) {
        ...
        VerifyMultiSignFile(file, fileWithSign, onSuccess, onError);
    };

    euSign.IsDataInSignedFileAvailable(fileWithSign, _onSuccess, onError);
}
...
function VerifyFileWithASiC(file, onSuccess, onError) {
    ...
    try {
        ...
        signContainerType =
            asicType == EU_ASIC_TYPE_S ? EUSignContainerType.ASiCS : EUSignContainerType.ASiCE;
        signContainerType |=
            asicSignType == EU_ASIC_SIGN_TYPE_CADES
                ? EUSignContainerType.CAdES
                : EUSignContainerType.XAdES;

        for (var i = 0; i < signsCount; i++) {
            var signedDataFiles = [];
            var signInfo = euSign.ASiCVerifyData(i, fileData);
            var signerCert = euSign.CtxASiCGetSignerInfo(context, i, fileData);
            ...
            for (var j = 0; j < refNames.length; j++) {
                if (refNames[j].length > 0 && refNames[j][0] == "#") continue;
...
function IsPAdES(file) {
    try {
        if (GetFileExtension(file) != "pdf") return false;
...
function IsXAdES(file) {
    try {
        if (GetFileExtension(file) != "xml") return false;
...
function VerifyFileWithPDF(file, onSuccess, onError) {
    ...
    try {
        ...
        for (var i = 0; i < signsCount; i++) {
            ...
            var signerCert = euSign.CtxPDFGetSignerInfo(context, i, fileData);
            var signType = euSign.PDFGetSignType(i, fileData);

            signInfo.data = fileData;
            signsInfos.push(
                CreateSignInfoResult(signInfo, signerCert, signType, signContainerType, true),
            );
        }
...
function VerifyFileWithXAdES(dataFile, signFile, onSuccess, onError) {
    ...
    try {
        if (dataFile) data = euSign.ReadFile(dataFile).GetData();
    ...
    try {
        ...
        if (xadesType == EU_XADES_TYPE_DETACHED) {
            ...
            references.push(new EndUserReference(dataFile.name, data));
        }

        for (var i = 0; i < signsCount; i++) {
            var signInfo = euSign.XAdESVerifyData(references, i, sign);
            var signerCert = euSign.CtxXAdESGetSignerInfo(context, i, sign);
...
function VerifyFilesWithExternalSign(files, onSuccess, onError) {
    ...
    var _onError = function (e) {
        ...
        VerifyFileWithExternalSign(files[1], files[0], onSuccess, function (e) {
            if (e.errorCode != EU_ERROR_PKI_FORMATS_FAILED) {
                onError(e);
                return;
            }
    ...
    VerifyFileWithExternalSign(files[0], files[1], onSuccess, _onError);
}
...
function VerifyFiles(data) {
    ...
    if (!s_verifyLargeFiles) {
        ...
        if (files.length == 2 && files[1].size > s_maxFileSize) {
            _onSuccess(
                new EUVerifyResult(
                    EUVerifyErrorCode.FileTooBig,
                    files[1],
    ...
    if (files.length == 1) {
        if (IsASiC(files[0])) {
            VerifyFileWithASiC(files[0], _onSuccess, _onError);
        } else if (IsPAdES(files[0])) {
            VerifyFileWithPDF(files[0], _onSuccess, _onError);
        } else if (IsXAdES(files[0])) {
            VerifyFileWithXAdES(null, files[0], _onSuccess, _onError);
        } else {
            VerifyFileWithInternalSign(files[0], _onSuccess, _onError);
        }
    } else {
        var xadesDataFile = null,
            xadesSignFile = null;
        if (IsXAdES(files[0])) {
        ...
        if (xadesDataFile != null && xadesSignFile != null) {
            VerifyFileWithXAdES(xadesDataFile, xadesSignFile, _onSuccess, _onError);
        } else {
            VerifyFilesWithExternalSign(files, _onSuccess, _onError);
        }
    }
}
